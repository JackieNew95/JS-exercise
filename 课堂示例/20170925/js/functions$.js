		...
		/*
			...
			参数说明
				...
				3、    tagName  规则：
					1、以字符开头[a~z]
					2、
				4、函数名  实现window.onload
			$函数的作用，快速获取元素
		 */
		
		function $(select,ranger){
			if(typeof select=='string'){//如果是字符串的话，表明其是选择器，按选择器处理
			...
			}else if(typeof select =='function'){//如果是函数的话，表明其是函数，按函数处理
				window.addEventListener('load',select);//函数重载
			}
		}
